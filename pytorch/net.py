...
class Net:
    ...
    def foward(self, w1, b1, x):
        return x * w1 + b1

    def epoch(self, output):
    ...
    def mini_batch(self, output, batch_id, data_x):
...
#     # print(w1, b1)
#     out = net.foward(w1, b1, train_x)
#     grad_w1, grad_b1, loss = net.epoch(out)
#     w1 -= lr * grad_w1
#     b1 -= lr * grad_b1
...
for epoch in range(80):
    ...
    for batch_id in range(0, int(N * 0.7), 10):
        out = net.foward(w1, b1, train_x[batch_id:batch_id+10])
        grad_w1, grad_b1, loss = net.mini_batch(out, batch_id, train_x[batch_id:batch_id+10])
        w1 -= lr * grad_w1
        b1 -= lr * grad_b1
...
def standardize(train):
    media = np.mean(train)
    Standardizing = np.sqrt(np.mean(np.power(train - media, 2)))
